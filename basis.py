...
class LGLBasis1D:
    ...
    def __init__(self, order):
        ...
        # self.nodes, self.weights = poly.legendre.leggauss(order)
    ...
    def set_deriv_matrix(self):
        self.derivative_matrix = cp.array(np.tensordot(self.inv_vandermonde, self.deriv_vandermonde, axes=([0], [0])))
    ...
    def set_quadratic_flux_matrix(self):
        ...
        local_order = 3 * (self.order - 1) // 2 + 10  # note: order must be odd, or round up
        gl_nodes, gl_weights = poly.legendre.leggauss(local_order)
        # Evaluate Legendre polynomials at finer grid
        ps = np.array([sp.legendre(s)(gl_nodes) for s in range(self.order)])
        ...
        # construct guess matrix
        # guess_matrix = np.array([[[
        #     self.weights[s]*self.weights[j]*self.weights[k] * sum(sum(
        #         (r+0.5)*(q+0.5)*sp.legendre(r)(self.nodes[s])*sp.legendre(r).deriv()(self.nodes[k]) *
        #           sp.legendre(q)(self.nodes[j])*sp.legendre(q)(self.nodes[k])
